...
MPolygon::MPolygon(const vector<Line*> &initLines):lines(initLines)
{
...
vector<Point> MPolygon::getVertexes() const
{
    return vertexes;
}

Point MPolygon::getCenter() const
{
    return center;
}

Point MPolygon::getHandlePoint() const
{
    return handle;
}

void MPolygon::setVertex(int idx, const Point &p)
{
...
void MPolygon::setHandlePointByRef(const Point &ref)
{
...
void MPolygon::translate(const Point &offset)
{
    for(Point &p:vertexes)
        p.translate(offset);
    for(Line *line:lines)
...
void MPolygon::rotate(double angle)
{
    for(Point &p:vertexes)
        p.rotate(center, angle);
    for(int i=0;i<lines.size();i++)
...
bool MPolygon::cut(const Point &leftDown, int width, int height)
{
...
bool MPolygon::isOn(const Point &p)
{
    return isOnPlain(p) || p.distanceTo(center)<=5 || p.distanceTo(handle)<=5;
}

bool MPolygon::isOnPlain(const Point &p)
{
    for(Line *line:lines)
        if(line->isOnPlain(p))
            return true;
...
const int MPolygon::h = 30; //handle长度
void MPolygon::calculateHandle()
{
    int minX = vertexes[0].getX();
    int maxX = vertexes[0].getX();
    int minY = vertexes[0].getY();
